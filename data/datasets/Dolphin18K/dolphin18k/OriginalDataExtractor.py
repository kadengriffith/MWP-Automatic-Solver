...
def extract_raw_problom(str_html):
    ...
    if i_problem_title_start != -1:
        i_problem_title_end = str_html.find(
            problem_text_end, i_problem_title_start)
        str_problem_title = str_html[i_problem_title_start +
                                     len(meta_problem_title):i_problem_title_end]
    ...
    if i_problem_text_start != -1:
        i_problem_text_end = str_html.find(
            problem_text_end, i_problem_text_start)
        str_problem_text = str_html[i_problem_text_start +
...
def generate_raw_question_file_multi_thread(input_file, out_raw_question_file, ithread=10):
    ...
    for i in range(0, ithread):
        ...
        # tid.start()
        try:
            tid = threading.Thread(target=assign_original_text, args=(
                split_qids[i], qid2index, word_probs_datas, ))
            tid.start()
            threads.append(tid)
        except:
...
if __name__ == "__main__":
    ...
    url_file = "dev_urls.json"
    i_thread = 10
    out_file = "dev_original.json"
